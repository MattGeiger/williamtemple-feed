// Backend model specifications - shared with frontend
// This file is a copy of frontend model-specs for backend usage
// Keeping them in sync for consistency
...
// Get model spec by model string
...
// Build OpenAI API parameters based on model
export function buildOpenAIParameters(model: string, maxTokens?: number): Record<string, any> {
  const modelSpec = getModelSpecByModel(model)
  
  const params: Record<string, any> = {}
  const resolvedMaxTokens = typeof maxTokens === 'number'
    ? maxTokens
    : modelSpec?.outputTokenLimit || 2048;
  
  // Determine which max tokens field to use
  const maxTokensField = modelSpec?.apiParameters?.maxTokensField || 'max_tokens'
  params[maxTokensField] = resolvedMaxTokens
