...
- **`document.startViewTransition` is absent on Safari 15.4**, so the theme switch there runs `runFallbackReveal` in `lib/theme-transition.ts` — a hold layer in the outgoing background, the class flip hidden beneath it, then a disc of the incoming background scaled from the trigger. It animates `transform` and `opacity` only; older WebKit will not interpolate `clip-path`. When testing a background colour for usability, check the **alpha component**, not a trailing `", 0)"` — FEED's dark background is true black, so `rgb(0, 0, 0)` reads as transparent under a naive sniff and the disc flashes white on every switch into dark. Covered by `src/test/theme-transition-fallback.test.ts`.
