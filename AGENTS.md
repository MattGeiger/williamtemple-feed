...
- **Model ids, prices, and language support live in one catalogue.**
  `packages/backend/src/services/ai/catalogue.ts` is authoritative for every
  model FEED can be configured to use. The providers read it directly and the
  configuration dialogs read it over `GET /api/ai-config/models`; both
  `model-specs.ts` copies are deleted, and a test in
...
- **Production release notes must not carry an "Unreleased" heading.** `ReleaseNotesCard` renders `docs/release-notes.md` directly when users click the version number. Before building a production frontend, remove the draft heading and ensure the first release entry describes the version being shipped. Developer-only `CHANGELOG.md` may retain its Unreleased section.
...
- **A retired brand field stays in the schema as optional; it is not deleted.** `identity.tagline` is no longer editable — "Food Equity & Efficient Delivery" spells FEED, so it is the product's name written out rather than agency copy, and `FEED_TAGLINE` is served regardless of what a payload holds. The field remains in `config-schema.ts` marked optional on purpose. `parseBrandConfig` rejects the whole payload on any unknown or invalid key, and a rejected payload does not degrade gracefully — `resolveBrand` falls back to the compiled default, so **every** appearance saved while the field was editable would silently lose its logos, colours and copy over one string nobody can set any more. The same reasoning applies to `capabilities`, which is likewise retained but no longer a wizard step. Retiring a field means: stop writing it, stop reading it, keep accepting it.
- **A brand may have no colour in it, and the accent may be a neutral family.** The role pools in `snap.ts` are exclusive by design — an accent searched chromatic ramps, a neutral searched neutral ones — and fixing that by *slot* rather than by the colour meant a grayscale identity had its primary replaced by whatever saturated family sat nearest. That is not a near miss, it is a different brand. `roleFor` picks the pool from the colour's own chroma instead, with `ACHROMATIC_CHROMA = 0.03`: Tailwind's neutral ramps top out at 0.046 and its chromatic ramps start at 0.013, so they overlap only among near-white tints, and every real brand colour sits far clear (WTH blue 0.120, WTH gold 0.160, St. Johns green 0.121, against St. Johns' near-black 0.000 and off-white 0.008). Coloured brands are unaffected. The exhaustive contrast proof now walks **234** themes rather than 153 — the 81 grayscale combinations are proved on the same terms, not assumed safe. Operational status colours are protected tokens and stay coloured throughout, so a grayscale UI still signals stock state in colour.
...
- **`document.startViewTransition` is absent on Safari 15.4**, so the theme switch there takes the crossfade path in `lib/theme-transition.ts`: a `.theme-crossfade` class is applied for the length of the change, the colours interpolate in place, and the class is removed so ordinary hover and focus transitions are untouched the rest of the time. Covered by `src/test/theme-transition-fallback.test.ts`, whose central assertion is that no overlay is added at all — because the first attempt at this fallback covered the viewport in the outgoing background colour and swept a disc of the incoming one across it. **A flat colour is not a frame, it is a blank**, so the whole dashboard vanished for the length of the sweep. That passed a unit test and looked fine in a harness holding two cards; it was obvious immediately on a real page. Animate a fallback against real content before believing it.
- **What actually blocks the circular reveal on Safari 15.4 is the absence of a pixel snapshot, and nothing else.** Two claims that sound like blockers are false, both measured on-device: `clip-path: circle()` interpolates perfectly there, under both CSS transitions and `Element.animate` (sampled clean and linear, 14.85% → 74.85%); and the reveal can be approximated without the API by cloning the DOM to get the second rendering — the clone keeps the outgoing tokens via a `.light`/`.dark` wrapper, since FEED's themes are pure custom properties. A prototype ran at a steady 60fps on the A8 with **zero** frames over 32ms. The real cost is the up-front layout of the second copy, which scales with node count: **552 nodes → 31ms, 2712 nodes → 150ms**, blocking, before the animation starts. `cloneNode` itself is free (1–2ms). So the trade is a genuine reveal with a stall proportional to page weight, against a crossfade with none — plus the edge cases a clone drags in (portalled dialogs live outside the cloned subtree, scroll offset must be reapplied, `<canvas>` clones blank). Do not re-derive these numbers; they are measured. The prototype is not in the tree.
